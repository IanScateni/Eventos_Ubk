    ...
    <div class="container">
            ...
            <?php foreach ($reservation as $data) {
                if ($data['type'] == '0') {?>
                    ...
                    <?php if (strstr($servicios, "Menú")) {
                        if ($data['menu'] == "") {?>
                            <p><strong>Menú Seleccionado: </strong> No se selecciono ningun plato</p>
                        <?php } else {
        ...
        <center>
            ...
            <div class="d-grid gap-2 col-6 mx-auto">
                <button type="button" class="btn btn-success" onclick="completedReserva('<?= $data['id']?>')">Finalizar</button>
            </div>
            <hr>
            <?php }else if ($data['status'] == "Pending"){ ?>
                <button type="button" class="btn btn-success" onclick="aprobarReserva('<?= $data['id']?>')">Aprobar</button>
                <button type="button" class="btn btn-danger" onclick="rechazarReserva('<?= $data['id']?>')">Rechazar</button>
            <hr>
            <?php } ?>
            </center>
        <div class="d-grid gap-2 col-4 mx-auto">
            <a class="btn btn-danger" type="button" href="javascript:close_tab();">Volver a las reservas</a>
        </div>
    </div><br>
</section>
<script type="text/javascript">
    ...
    function aprobarReserva(id_reserva){
        alertify.confirm('Aprobar reservación', '¿Seguro de aprobar esta reserva?', function(){
            $.ajax({
                type: "POST",
                data: "id_reserva=" + id_reserva,
                url: "/helpers/approvedReserved.php",
                success:function(r){
                   if (r==1) {
                        alertify.success("Aprobado con éxito");
                        location.reload();
                   } else {
                        alertify.error("No se pudo aprobar la reserva, intente de nuevo");
                   }
                }
            });
        }
        , function(){
        }).set('labels', {ok: 'Aprobar', cancel:'Cerrar'});
    }
    function rechazarReserva(id_reserva){
        alertify.confirm('Rechazar reservación', '¿Seguro de rechazar esta reserva?', function(){
            $.ajax({
                type: "POST",
                data: "id_reserva=" + id_reserva,
                url: "/helpers/rejectedReserved.php",
                success:function(r){
                   if (r==1) {
                        alertify.success("La reserva quedo rechazada");
                        location.reload();
                   } else {
                        alertify.error("No se pudo rechazar la reserva, intente de nuevo");
                   }
                }
            });
        }
        , function(){
        }).set('labels', {ok: 'Rechazar', cancel:'Cerrar'});
    }
    function completedReserva(id_reserva){
        alertify.confirm('Finalizar reservación', '¿Seguro de finalizar esta reserva?', function(){
            $.ajax({
                type: "POST",
                data: "id_reserva=" + id_reserva,
                url: "/helpers/completedReserved.php",
                success:function(r){
                   if (r==1) {
                        alertify.success("La reserva quedo finalizada");
                        location.reload();
                   } else {
                        alertify.error("No se pudo finalizar la reserva, intente de nuevo");
                   }
                }
            });
        }
        , function(){
        }).set('labels', {ok: 'Finalizar', cancel:'Cerrar'});
    }
</script>